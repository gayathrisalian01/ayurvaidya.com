...
$stmt = $pdo->prepare("SELECT consultation_form FROM consultation_form WHERE consultation_id = :id");
...
header('Content-Disposition: inline; filename="consultation.pdf"');
header('Content-Description: SDM-AYURVAIDYA');
header('Content-Transfer-Encoding: binary');
header('Cache-Control: must-revalidate, post-check=0, pre-check=0');
echo $form['consultation_form'];
